...
namespace dx3d
{
	class World final : public Base
	{
		public:
			...
			template <typename T> 
			T* createGameObject() requires IsRegistered<GameObject, T> //requires clause to ensure that T is a registered GameObject type
			{
			...
			template <typename T> requires IsRegistered<Component, T> //requires clause to ensure that T is a registered Component type
