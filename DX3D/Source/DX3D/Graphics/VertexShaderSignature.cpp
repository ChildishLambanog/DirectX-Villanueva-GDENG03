...
{
	if (!desc.vsBinary) DX3DLogThrowInvalidArgument("No shader binary provided!");
	if (desc.vsBinary->getType() != ShaderType::VertexShader)
		DX3DLogThrowInvalidArgument("The 'vsBinary' member is not a valid vertex shader binary!");
	...
	for (auto i : std::views::iota(0u, m_numElements))
	{
		DX3DGraphicsLogThrowOnFail(m_shaderReflection->GetInputParameterDesc(i, &params[i]), 
			"ID3D11ShaderReflection::GetInputParameterDesc has failed!");
	}
...
dx3d::BinaryData dx3d::VertexShaderSignature::getInputElementData() const noexcept
{
	return
	{
		m_elements,
