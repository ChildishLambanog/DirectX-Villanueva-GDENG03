...
dx3d::ConstantBuffer::ConstantBuffer(const ConstantBufferDesc& desc, const GraphicsResourceDesc gDesc) :
	GraphicsResource(gDesc), m_bufferSize(desc.bufferSize)
{
	if (!desc.bufferSize) DX3DLogThrowInvalidArgument("Buffer size must be a non-zero value.");
