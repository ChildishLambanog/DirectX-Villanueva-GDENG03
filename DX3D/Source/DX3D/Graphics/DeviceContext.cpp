...
void dx3d::DeviceContext::updateConstantBuffer(const ConstantBuffer& buffer, const void* data)
{
	...
	DX3DGraphicsLogThrowOnFail(m_context->Map(buf, 0, D3D11_MAP_WRITE_DISCARD, 0, &mapped), "ID3D11DeviceContext::Map has failed.");
	std::memcpy(mapped.pData, data, buffer.m_bufferSize);
